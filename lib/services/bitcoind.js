...
Bitcoin.prototype._notifyAddressTxidSubscribers2 = function(txid, transaction) {
  var addresses = {};
  var network = this.node.network.name == 'mainnet'? bitcoinjs.networks.bitcoin: bitcoinjs.networks.testnet;

  /**
   * ToDo: get address from input
   */
  for (var i = 0; i < transaction.ins.length; i++) {
    ...
    if (input.script) {      
      var inputAddress = "";
      switch (bitcoinjs.script.classifyInput(input.script)) {
        case 'pubkeyhash':
          var inScript = bitcoinjs.script.pubKeyHash.input.decode(input.script);
          var outScript = bitcoinjs.script.pubKeyHash.output.encode(bitcoinjs.crypto.hash160(inScript.pubKey));
          inputAddress = bitcoinjs.address.fromOutputScript(outScript, network); 
          break;
        case 'scripthash':     
          var inScript = bitcoinjs.script.scriptHash.input.decode(input.script);
          var outScript = bitcoinjs.script.scriptHash.output.encode(bitcoinjs.crypto.hash160(inScript.redeemScript));
          inputAddress = bitcoinjs.address.fromOutputScript(outScript, network);
          break;
        case 'nonstandard':
          if(input.witness.length > 0){
            if(bitcoinjs.script.witnessPubKeyHash.input.check(input.witness)){
                var inScript = bitcoinjs.script.witnessPubKeyHash.input.decodeStack(input.witness);
                var outScript = bitcoinjs.script.witnessPubKeyHash.output.encode(bitcoinjs.crypto.hash160(inScript.pubKey));
                inputAddress = bitcoinjs.address.fromOutputScript(outScript, network);  
            }
            else if(bitcoinjs.script.witnessScriptHash.input.check(input.witness)){
                var inScript = bitcoinjs.script.witnessScriptHash.input.decodeStack(input.witness);
                var outScript = bitcoinjs.script.witnessScriptHash.output.encode(bitcoinjs.crypto.sha256(inScript.redeemScript));
                inputAddress = bitcoinjs.address.fromOutputScript(outScript, network);  
            } 
          }
        default:
      ...
      if (inputAddress) {
        addresses[inputAddress]=inputAddress;
      }
    }
  }

  for (var j = 0; j < transaction.outs.length; j++) {
    var output = transaction.outs[j];  
    if (output.script) {
        try{
          var outputAddress = bitcoinjs.address.fromOutputScript(output.script);       
        if (outputAddress) {
          addresses[outputAddress] = outputAddress;
        }        
      }
      catch(e){
...
Bitcoin.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    var tx = bitcoinjs.Transaction.fromBuffer(message);
    var txid = tx.getId();
    self._notifyAddressTxidSubscribers2(txid, tx);
...
Bitcoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  ...
  function transformUnspentOutput(delta) {
    try {
      var addressBech32 = bitcoinjs.address.fromBech32(delta.address);
      var script = '';
      if (addressBech32.data.length === 20) script = bitcoinjs.script.witnessPubKeyHash.output.encode(addressBech32.data)
      if (addressBech32.data.length === 32) script = bitcoinjs.script.witnessScriptHash.output.encode(addressBech32.data)   
      return {
...
// -- for bitcore 4.
// bitcore-node/lib/services/adddress/index.js 
Bitcoin.prototype.getAddressesSummary = function (addresses, options, callback) {
    var self = this;
    ...
    async.eachSeries(addresses, function (address, next) {
        self.getAddressSummary(address, options, function (err, summary) {
            if (err) {
                return next(err);
            }
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = bitcoinjs.Block.fromHex(response.result);
  ...
  if (tx) {
    ...
    self._tryAllClients(function(client, done) {
      client.getRawTransaction(txid, function(err, response) {
        ...
        var tx = bitcoinjs.Transaction.fromHex(response.result);
